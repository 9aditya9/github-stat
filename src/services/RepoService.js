export const GetRepos = async({username, limit=10, currentPage=1}) => { 
    const res = await fetch(`https://api.github.com/users/${username}/repos?page=${currentPage}&per_page=${limit}`);
    ...
    if (res.status === 200) {
        ...
        return {data: data, headers: res.headers, next: next, prev: prev, first: first, last: last};
    } else {
...
export const fetchRepos = async({username, limit, selectedPage}) => {
    const res = await fetch(`https://api.github.com/users/${username}/repos?per_page=${limit}&page=${selectedPage}`);
